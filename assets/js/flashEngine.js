/* ============================================================================
   CISM  ::  flashEngine.js  —  Rapid Recall Flashcards
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating CISM.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (cism.flash.v1) keyed by card id,
...
(function () {
  ...
  /* Domain id list derived from the single source of truth so this engine works
     for any domain count (CISM has 4). Never hardcode 5 / 500 / [1..5] here. */
  function DOMS() { return (S.domainMeta || []).map(function (d) { return d.id; }); }
  function TOTAL_CARDS() { return DOMS().length * DECK_SIZE; }

  function ver() { return window.CISM_VER ? "?v=" + window.CISM_VER : ""; }
